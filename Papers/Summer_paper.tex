...
%I use a Monte Carlo scattering experiment to determine the set of parameters that result in a test particle being accelerated to the velocity observed in \citet{cald14}. Applying these parameters to an N-body integration using AMUSE, and replacing the test particle with an extended globular cluster, I find that neither a typical large cluster of  $r_{h}$ = 6 pc nor a compact cluster of $r_{h}$ = 1 pc survive the optimum close encounter with the black hole system. Therefore, I find no evidence supporting the globular cluster reported in \citet{cald14} having been accelerated by a supermassive binary black hole.


%Through a Monte Carlo scattering experiment, I determine the parameters of a supermassive binary black hole necessary to impart a positive energy kick to a massive test particle, thereby ejecting it, or removing energy from the particle, causing it to be captured by the binary. 
\end{abstract}
...
\bibitem[McMillan et al.(2012)]{McM12}
